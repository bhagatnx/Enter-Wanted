...
export class EnterWanted {
    ...
    async getEmployeeInfo(): Promise<EnterWanted> {
        ...
        let Oai = await (
          await this.driver.findElement(this.oriInput)
        ).getAttribute("value");
        let Name = await (
          await this.driver.findElement(this.namInput)
        ).getAttribute("value");
        let Sex = await (
            await this.driver.findElement(this.sexInput)
          ).getAttribute("value");
        let Race = await (
          await this.driver.findElement(this.racInput)
        ).getAttribute("value");
        let Height = await (
            await this.driver.findElement(this.hgtInput)
          ).getAttribute("value");
        let Weight = await (
          await this.driver.findElement(this.wgtInput)
        ).getAttribute("value");
        let Hair = await (
          await this.driver.findElement(this.haiInput)
        ).getAttribute("value");
        let Offense = await (
          await this.driver.findElement(this.offInput)
        ).getAttribute("value");
        let DOW = await (
